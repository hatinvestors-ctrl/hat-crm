...
describe('A/B/C/D — fresh off-market lead is never blocked from Property Analysis', () => {
  ...
  it('B. missing Evaluation Price (asking_price) does not block runPropertyOnly — it only calls generate-comps, never checks asking_price', () => {
    ...
    const end = cardSrc.indexOf('// Re-run only core analysis with ARV and/or reno overrides')
  ...
  it('D. missing ARV does not block runPropertyOnly — no ARV reference/requirement inside it', () => {
    const start = cardSrc.indexOf('const runPropertyOnly = async () => {')
    const end = cardSrc.indexOf('// Re-run only core analysis with ARV and/or reno overrides')
    const body = cardSrc.slice(start, end)
    expect(body).not.toMatch(/lead\.arv|arvForCore|resolvedArv/)
  })
})

describe('E/F/G/H — no fabricated values written by Stage A', () => {
  const start = cardSrc.indexOf('const runPropertyOnly = async () => {')
  const end = cardSrc.indexOf('// Re-run only core analysis with ARV and/or reno overrides')
  const body = cardSrc.slice(start, end)
  ...
  it('F. no fake Seller Asking is created', () => {
    expect(body).not.toMatch(/seller_asking_price|seller_intelligence/)
  })
  it('G. no fake Renovation is created', () => {
    expect(body).not.toMatch(/renovation_cost:/)
  })
  it('H. no fake ARV is created', () => {
    expect(body).not.toMatch(/arv:/)
  })
  it('the ONLY field runPropertyOnly writes to the database is ai_notes', () => {
    const updateCalls = body.match(/supabase\.from\('leads'\)\.update\(\{[^}]*\}\)/g) || []
    expect(updateCalls.length).toBe(1)
    expect(updateCalls[0]).toMatch(/ai_notes: fullNotes/)
    expect(updateCalls[0]).not.toMatch(/arv|asking_price|renovation_cost|mao|deal_analysis/)
  })
})
...
describe('I/J — orchestration correctly routes Stage A vs Stage B, never runs Stage B with missing inputs', () => {
  it('I. generate-comps can run in the property-only path (callFn reused, not a new AI call)', () => {
    ...
    const end = cardSrc.indexOf('// Re-run only core analysis with ARV and/or reno overrides')
    const body = cardSrc.slice(start, end)
    expect(body).toMatch(/callFn\('generate-comps', \{ lead: leadWithContext \}\)/)
  })
...
describe('M — real renovation_cost = 0 remains valid and distinct from null (unchanged)', () => {
  it('computeAnalysisReadiness still treats 0 as present, null/undefined as missing', () => {
    const src = fs.readFileSync('src/components/lead-detail/DealAnalysisCard.jsx', 'utf8')
    expect(src).toMatch(/const renoIsMissing = lead\?\.renovation_cost == null \/\/ 0 is present, per Part 5/)
  })
})
...
describe('AA/AB — protected financial logic untouched', () => {
  it('runPropertyOnly and handleAnalyzeClick call no canonical financial function', () => {
    ...
    const end = cardSrc.indexOf('// Re-run only core analysis with ARV and/or reno overrides')
    const body = cardSrc.slice(start, end)
    expect(body).not.toMatch(/calculateFlipMAO|calculateBrrrrMAO|computeFlipResult|computeBrrrrResult|computeStrategyRecommendation/)
  })
