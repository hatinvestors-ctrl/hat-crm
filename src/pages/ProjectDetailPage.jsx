...
// AI Investor Analysis section (BRRRR projects)
function BrrrrAIAnalysis({ lead, calc, onSave }) {
  const [running,  setRunning]  = useState(false)
  const [error,    setError]    = useState(null)

  if (!lead || !calc) return null

  const analysis = lead.notes || ''
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      const res = await fetch('/.netlify/functions/generate-brrrr-analysis', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ address, calc }),
      })
      const data = await res.json()
      if (!res.ok || !data.ok) throw new Error(data.error || 'Analysis failed')
      await onSave(data.analysis)
    } catch (e) {
      setError(e.message)
    } finally {
      setRunning(false)
  ...
  return (
    <div className="mt-4 rounded-xl border border-[color:var(--color-line)] bg-[color:var(--color-bg-elev)] overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 border-b border-[color:var(--color-line)]">
        <div>
          <div className="text-[12px] font-semibold text-[color:var(--color-text)]">✦ AI Investor Analysis</div>
          <div className="text-[10.5px] text-[color:var(--color-text-dim)] mt-0.5">
            BRRRR verdict · rent sensitivity · risks · action items
          </div>
        </div>
        <button
          onClick={handleGenerate}
          disabled={running}
          className={`shrink-0 text-[11px] font-semibold px-3 py-1.5 rounded transition-opacity disabled:opacity-50 ${
            analysis
              ? 'border border-[color:var(--color-line)] text-[color:var(--color-text-muted)] hover:text-[color:var(--color-text)] hover:border-[color:var(--color-accent)]'
              : 'bg-[color:var(--color-accent)] text-white hover:opacity-90'
          }`}
        >
          {running ? 'Analyzing…' : analysis ? '↺ Re-run Analysis' : '✦ Generate Analysis'}
        </button>
      </div>

      {error && (
        <div className="px-4 py-2 text-[11.5px] text-[color:var(--color-danger-text)] bg-[color:var(--color-danger-soft)]">
          {error}
        </div>
      )}

      {running && (
        <div className="px-4 py-8 text-center text-[12px] text-[color:var(--color-text-dim)] animate-pulse">
          Running AI analysis on deal numbers…
        </div>
      )}

      {!running && analysis ? (
        <div className="p-4">
          <pre className="text-[11.5px] leading-relaxed text-[color:var(--color-text-muted)] whitespace-pre-wrap font-mono">{analysis}</pre>
        </div>
      ) : !running && !analysis && !error ? (
        <div className="px-4 py-8 text-center text-[12px] text-[color:var(--color-text-dim)]">
          Click <strong>✦ Generate Analysis</strong> to get an AI investor review of this BRRRR deal.
        </div>
      ) : null}
    </div>
  )
