  ...
  return (
    ...
    >
      <div className="px-3.5 py-3">
        ...
        {/* Capability #17.1 — "Why now" one-liner, generated by
            getActionReason() from the SAME facts classifyLeadV2()/
            classifyLead() already used (see src/lib/actionReason.js).
            Compact, single line, no extra card. Falls back to the plain
            "Reason" line only if a reason object couldn't be built. */}
        {actionReason ? (
          <div className="text-[11px] text-[color:var(--color-text-muted)] mt-2 leading-snug">
            <span className="font-semibold text-[color:var(--color-text)]">Why now: </span>
            <span className="line-clamp-2">{actionReason.reason}</span>
          </div>
        ) : item.reason && (
          <div className="text-[11px] text-[color:var(--color-text-muted)] mt-2 leading-snug">
