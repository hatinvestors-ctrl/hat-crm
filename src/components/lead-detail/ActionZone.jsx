...
import { resolvePrimaryPriceGap } from '../../lib/acquisitionDecisionPresentation'
...
function smartActions(lead, staticActions) {
  ...
  // UX V2.1, Part 7/18 — consolidated onto the SAME strategy-aware price
  // gap the primary Overview hero uses (resolvePrimaryPriceGap,
  // acquisitionDecisionPresentation.js), instead of this component's own
  // independent copy that always subtracted Flip Max Buy regardless of
  // which strategy is actually preferred. No new math — same
  // computeFlipResult/computeBrrrrResult/computeStrategyRecommendation
  // facts, just no longer duplicated.
  const flip = computeFlipResult(lead)
  const brrrr = computeBrrrrResult(lead)
  const strategyRec = computeStrategyRecommendation(flip, brrrr)
  const priceGap = resolvePrimaryPriceGap({ flip, brrrr, strategyRec })
  const gap = priceGap ? priceGap.gap * (priceGap.withinBuyRange ? -1 : 1) : null
...
export function smartHint(lead, staticHint) {
  ...
  // contradiction — "Seller is $15,959 above Flip Max Buy" while the
  // hero recommended negotiating toward the BRRRR target). No new math.
  const flip = computeFlipResult(lead)
  const brrrr = computeBrrrrResult(lead)
  const strategyRec = computeStrategyRecommendation(flip, brrrr)
  const priceGap = resolvePrimaryPriceGap({ flip, brrrr, strategyRec })
  if (priceGap && !priceGap.withinBuyRange) {
    return `Seller is ${fmtFull(priceGap.gap)} above ${priceGap.targetLabel}. Decide: negotiate, schedule a follow-up, or move on.`
  }
