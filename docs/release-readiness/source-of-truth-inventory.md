...
## Workflow
`lead.status`, `lead.follow_up_date`. `TERMINAL_STATUSES` (`src/lib/constants.js`) structurally excludes leads from Action Center at the query level. `classifyFollowUpDate()`/`daysOverdue()` (`followUpTiming.js`) — `America/New_York`, date-only comparison, no UTC drift. Action Center category: `classifyLeadV2()` (`src/pages/ActionCenterPage.jsx`) — lives in a page component, but **Finding F4 is now closed**: it's a pure function with no JSX evaluated at import time, so it's directly importable and unit-testable under vitest exactly like a lib function (confirmed and covered — see `test/actionCenterClassification.test.js`, System Validation Review). `classifyLeadV2`'s `expectedProfit`/`maxOffer` fields read `lead.deal_analysis.profit` (AI-generated) and `lead.mao` (legacy formula) — see Defect D1 in `RELEASE-READINESS.md` for why this is a real cross-screen consistency concern with the canonical Flip/BRRRR pipeline.
