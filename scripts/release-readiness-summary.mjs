...
const CATEGORY_LABELS = {
  ...
  'dealOpportunitySummary.test.js': 'SMALL CHANGE #3 FROM SEP 4 BASELINE — ACTIONABLE ACQUISITION DECISION / DEAL OPPORTUNITY SUMMARY (presentation-only — computeStrategyRecommendation\'s own "NONE" conclusion, dealExplanation.js, byte-unchanged, is never mutated; acquisitionDecisionPresentation.js\'s existing PASS branch gains ONE new presentation state, PASS_NEGOTIABLE, that fires ONLY when a genuinely feasible Flip/BRRRR Max Buy exists below the current price [flip.maoFeasible/brrrr.mao, already-computed fields — no new threshold], headlined NEGOTIATE with a "Pass at current price" secondary line, distinct from a true hard PASS [no viable price at any strategy] which renders unchanged; new buildDealOpportunitySummary() evaluates the EXISTING computeFlipBreakdown/computeBrrrrBreakdown [calculations.js, UNCHANGED, same functions computeFlipResult/computeBrrrrResult already call internally] at the canonical Max Buy for READ-ONLY display [profit at Max Buy, cash flow/cash left in at Max Buy] — same precedent as dealExplanation.js\'s own computeFlipDownsideSensitivity, nothing persisted, no new formula; DecisionHero.jsx renders a compact FLIP/BRRRR comparison [current price/profit-or-cash-flow now vs at Max Buy, price gap, honest "Rent estimate needed" when BRRRR isn\'t computable] plus a Recommended Action line distinguishing the existing Suggested Offer [flip.currentOffer] from Max Buy [flip.mao] — never conflated; 1463 Spring Street [$110K ask, $185K ARV, $45K rehab, no rent] and Fouraker [$200K ask, $285K ARV, $20K rehab] regressions both verified against the real, unmodified engine — Flip Max Buy $83,164/$195,104 respectively, profit now $1,232/Fouraker\'s real figure, profit at Max Buy reconciling to the engine\'s own target profit; true hard-pass and already-within-range cases confirmed unaffected; Small Change #1/#2 confirmed byte-unchanged; all six protected files confirmed byte-unchanged)',
}
